...
par(mfrow=c(1,2)) 
plot(mean_notnorm_low_byyear$ger_secondary,mean_notnorm_low_byyear$molten.flfpr,col="red",ylab="FLFPR_LowIncome",xlab="GER_Secondary",pch = 1)
plot(mean_notnorm_high_byyear$ger_secondary,mean_notnorm_high_byyear$molten.flfpr,col="blue",ylab="FLFPR_HighIncome",xlab="GER_Secondary",pch = 2)
...
par(mfrow=c(1,2)) 
plot(mean_notnorm_low_byyear$seatprop,mean_notnorm_low_byyear$molten.flfpr,col="red",ylab="FLFPR_LowIncome",xlab="Seat Proportion in National government",pch = 1)
plot(mean_notnorm_high_byyear$seatprop,mean_notnorm_high_byyear$molten.flfpr,col="blue",ylab="FLFPR_HighIncome",xlab="Seat Proportion in National government",pch = 2)
...
#Classic ggplot
tiff("test.tiff1", units="in", width=5, height=5, res=300)
ggplot(data=mean_by_country_cast,aes(x=mean_by_country_cast$ml_length1, y=mean_by_country_cast$molten.flfpr, size = mean_by_country_cast$molten.gdp, color=mean_by_country_cast$Country)) +
  ...
  geom_text( data=mean_by_country_cast, aes(mean_by_country_cast$ml_length1,mean_by_country_cast$molten.flfpr , label = mean_by_country_cast$Country), colour = I(alpha("black", 0.85)), size = 1 )+
    theme(legend.position="none") + xlab("Maternity Leave Length") + ylab("Female Labor Force Participation Rate")
dev.off()
```
...
#Classic ggplot
main_cast_final_country=aggregate(main_cast_final[, 3:10], list(main_cast_final$Country), mean,na.rm=TRUE, na.action=NULL)
tiff("test.tiff", units="in", width=5, height=5, res=300)
ggplot(data=main_cast_final_country,aes(x=main_cast_final_country$fertility, y=main_cast_final_country$molten.flfpr, size = main_cast_final_country$molten.gdp, color=main_cast_final_country$Group.1)) +
    geom_point(data=main_cast_final_country,alpha=0.7) +
    scale_size(range = c(1.4, 19), name="FLFPR") +
    scale_color_viridis(discrete=TRUE, guide=FALSE) +
    theme_ipsum() +
    ggtitle("FLFPR vs Fertility")+
  geom_text( data=main_cast_final_country, aes(main_cast_final_country$fertility,main_cast_final_country$molten.flfpr , label = main_cast_final_country$Group.1), colour = I(alpha("black", 0.85)), size = 1 )+
    theme(legend.position="none") + xlab("Fertility") + ylab("Female Labor Force Participation")
dev.off()
```
```{r}
setwd("~/Documents/GitHub/dav_final/GenderStatsData")
